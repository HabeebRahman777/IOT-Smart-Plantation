...
const Light = ({client}) => {
  const [light, setLight] = useState(10); // Default temperature
  ...
  useEffect(() => {
    ...
    client.on("message", (topic, message) => {
      if (topic === "esp32/light") {
        const receivedLight = parseFloat(message.toString()); // Convert MQTT message to number
        setLight(receivedLight);
      }
    });
  }, [client]);
  ...
  const data = [{ name: "Temp", value: light, fill: getColor(light) }];

  return (
    <div className="bg-white/20 backdrop-blur-md shadow-lg rounded-2xl p-5 flex flex-col items-center w-64">
      <h2 className="text-gray-800 text-xl font-extrabold mb-2">Light Intensity</h2>
      ...
      <h2 className="text-green text-2xl font-bold mt-3">{light} Lux</h2>
    </div>
  );
};
