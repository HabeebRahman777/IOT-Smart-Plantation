...
const Temperature = ({client}) => {
  const [temp, setTemp] = useState(10); // Default temperature
  ...
  useEffect(() => {
    ...
    client.on("message", (topic, message) => {
      if (topic === "esp32/temperature") {
        const receivedTemp = parseFloat(message.toString()); // Convert MQTT message to number
        setTemp(receivedTemp);
      }
    });
  }, [client]);
  ...
  const data = [{ name: "Temp", value: temp, fill: getColor(temp) }];

  return (
    <div className="bg-white/20 backdrop-blur-md shadow-lg rounded-2xl p-5 flex flex-col items-center w-64">
      <h2 className="text-gray-800 text-xl font-extrabold mb-2">Temperature</h2>
      ...
      <h2 className="text-green text-2xl font-bold mt-3">{temp}°C</h2>
    </div>
  );
};
