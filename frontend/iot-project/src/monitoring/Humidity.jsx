...
const Humidity = ({client}) => {
  const [humidity, setHumidity] = useState(10); // Default temperature

  useEffect(() => {
    if (!client) return;

    const topic = "esp32/humidity";
    ...
    client.on("message", (topic, message) => {
      if (topic === "esp32/humidity") {
        const receivedHumidity = parseFloat(message.toString()); // Convert MQTT message to number
  ...
  const data = [{ name: "Temp", value: humidity, fill: getColor(humidity) }];

  return (
    <div className="bg-white/20 backdrop-blur-md shadow-lg rounded-2xl p-5 flex flex-col items-center w-64">

      <h2 className="text-gray-800 text-xl font-extrabold mb-2">Humidity</h2>
      ...
      <h2 className="text-green text-2xl font-bold mt-3">{humidity} g/m3</h2>
    </div>
  );
};
